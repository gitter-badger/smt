...
if (scalar(@ARGV) < 7 || $help) {
    print "syntax: clean-corpus-n.perl corpus l1 l2 clean-corpus min max fract [lines retained file]\n";
    exit;
}
...
print STDERR "clean-corpus.perl: processing $corpus.$l1 & .$l2 to $out, cutoff $min-$max, fraction $fract\n";
...
while(my $f = <F>) {
  ...
  next if $ec < $min;
  next if $fc < $min;
  next if !$ignore_ratio && $ec/$fc > $fract;
  next if !$ignore_ratio && $fc/$ec > $fract;
